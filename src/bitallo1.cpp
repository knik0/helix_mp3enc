/* ***** BEGIN LICENSE BLOCK *****  
 * Source last modified: $Id: bitallo1.cpp,v 1.1 2005/07/13 17:22:20 rggammon Exp $ 
 *   
...
CBitAllo1::BitAlloInit ( BA_CONTROL & bac )
{
    ...
    for ( i = 0; i < 128; i++ )
    {
        ...
        look_34igain[i] =
            ( float ) ( 1.0 / pow ( look_gain[i], ( 3.0 / 4.0 ) ) );
    }

/* inverse quant */
    for ( i = 0; i < 256; i++ )
        look_ix43[i] = ( float ) ( i * pow ( i, ( 1.0 / 3.0 ) ) );

    for ( i = 0; i < 21; i++ )
        look_log_cbw[i] = ( float ) ( 10.0 * log10 ( nBand_l[i] ) );
